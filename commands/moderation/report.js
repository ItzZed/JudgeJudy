...
module.exports = {
	...
	run: async (client, message, args) => {
		...
		if (!args[1])
			return message.reply("Please provide a reason for the report").then(m => m.delete(5000));

		const channel = client.channels.cache.find(channel => channel.name === "reports");

		// No channel found
		if (!channel)
			return message.reply("Couldn't find a `#reports` channel").then(m => m.delete(5000));
